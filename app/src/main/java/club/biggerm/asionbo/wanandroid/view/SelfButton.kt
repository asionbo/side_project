package club.biggerm.asionbo.wanandroid.view
...
/**
 * Created by asionbo on 2018/9/13.
 */

class SelfButton @JvmOverloads constructor(
    ...
    init {
        context.obtainStyledAttributes(attrs, R.styleable.SelfButton).apply {
            mShapeMode = getInt(R.styleable.SelfButton_sb_shapeMode, 0)
            mFillColor = getColor(R.styleable.SelfButton_sb_fillColor, 0xFFFFFFFF.toInt())
            mPressedColor = getColor(R.styleable.SelfButton_sb_pressedColor, 0xFF666666.toInt())
            mStrokeColor = getColor(R.styleable.SelfButton_sb_strokeColor, 0)
            mStrokeWidth = getDimensionPixelSize(R.styleable.SelfButton_sb_strokeWidth, 0)
            mCornerRadius = getDimensionPixelSize(R.styleable.SelfButton_sb_cornerRadius, 0)
            mCornerPosition = getInt(R.styleable.SelfButton_sb_cornerPosition, -1)
            mActiveEnable = getBoolean(R.styleable.SelfButton_sb_activeEnable, false)
            mDrawablePosition = getInt(R.styleable.SelfButton_sb_drawablePosition, -1)
            mStartColor = getColor(R.styleable.SelfButton_sb_startColor, 0xFFFFFFFF.toInt())
            mEndColor = getColor(R.styleable.SelfButton_sb_endColor, 0xFFFFFFFF.toInt())
            mOrientation = getColor(R.styleable.SelfButton_sb_orientation, 0)
            recycle()
        }
    }
                ...
                drawable?.let {
                    ...
                    when(mDrawablePosition){
                        ...
                        0,2 ->{
                            ...
                            contentWidth = drawableWidth+textWidth+drawablePadding
                        ...
                        1,3 ->{
                            ...
                            //单行高度
                            val singleLineHeight = Math.ceil(fm.descent.toDouble() - fm.ascent.toDouble()).toFloat()
